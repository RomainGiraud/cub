...
bool CDDSImage::save(std::string filename, bool flipImage)
{
    ...
    if (is_compressed())
    {
        ...
        if (m_format == gl::COMPRESSED_RGBA_S3TC_DXT1_EXT)
            ddsh.ddspf.dwFourCC = FOURCC_DXT1;
        if (m_format == gl::COMPRESSED_RGBA_S3TC_DXT3_EXT)
            ddsh.ddspf.dwFourCC = FOURCC_DXT3;
        if (m_format == gl::COMPRESSED_RGBA_S3TC_DXT5_EXT)
...
bool CDDSImage::upload_texture1D()
{
    ...
    if (is_compressed())
    {
        ...
        if (CompressedTexImage1DARB == NULL)
        {
        ...
        if (glCompressedTexImage1DARB == NULL)
            return false;
        */
        gl::CompressedTexImage1D(gl::TEXTURE_1D, 0, m_format, 
        ...
        for (unsigned int i = 0; i < baseImage.get_num_mipmaps(); i++)
        {
            ...
            gl::CompressedTexImage1D(gl::TEXTURE_1D, i+1, m_format, 
    ...
    else
    {
        ...
        gl::TexImage1D(gl::TEXTURE_1D, 0, m_components, baseImage.get_width(), 0,
            m_format, gl::UNSIGNED_BYTE, baseImage);
        ...
        for (unsigned int i = 0; i < baseImage.get_num_mipmaps(); i++)
        {
            ...
            gl::TexImage1D(gl::TEXTURE_1D, i+1, m_components, 
                mipmap.get_width(), 0, m_format, gl::UNSIGNED_BYTE, mipmap);
        }
...
bool CDDSImage::upload_texture2D(unsigned int imageIndex, GLenum target)
{
    ...
    assert(target == gl::TEXTURE_2D || target == gl::TEXTURE_RECTANGLE_ARB ||
    ...
    if (is_compressed())
    {
        ...
        if (glCompressedTexImage2DARB == NULL)
            return false;
        */
        gl::CompressedTexImage2D(target, 0, m_format, image.get_width(), 
        ...
        for (unsigned int i = 0; i < image.get_num_mipmaps(); i++)
        {
            ...
            gl::CompressedTexImage2D(target, i+1, m_format, 
    ...
    else
    {
        ...
        gl::TexImage2D(target, 0, m_components, image.get_width(), 
            image.get_height(), 0, m_format, gl::UNSIGNED_BYTE, 
            image);
        ...
        for (unsigned int i = 0; i < image.get_num_mipmaps(); i++)
        {
            ...
            gl::TexImage2D(target, i+1, m_components, mipmap.get_width(), 
                mipmap.get_height(), 0, m_format, gl::UNSIGNED_BYTE, mipmap); 
        }
...
bool CDDSImage::upload_texture3D()
{
    ...
    if (is_compressed())
    {
        ...
        if (glCompressedTexImage3DARB == NULL)
            return false;
        */
        gl::CompressedTexImage3D(gl::TEXTURE_3D, 0, m_format,  
        ...
        for (unsigned int i = 0; i < baseImage.get_num_mipmaps(); i++)
        {
            ...
            gl::CompressedTexImage3D(gl::TEXTURE_3D, i+1, m_format, 
    ...
    else
    {
        ...
        */
        ...
        gl::TexImage3D(gl::TEXTURE_3D, 0, m_components, baseImage.get_width(), 
            baseImage.get_height(), baseImage.get_depth(), 0, m_format, 
            gl::UNSIGNED_BYTE, baseImage);
        ...
        for (unsigned int i = 0; i < baseImage.get_num_mipmaps(); i++)
        {
            ...
            gl::TexImage3D(gl::TEXTURE_3D, i+1, m_components, 
                mipmap.get_width(), mipmap.get_height(), mipmap.get_depth(), 0, 
                m_format, gl::UNSIGNED_BYTE,  mipmap);
        }
...
{
    return upload_texture2D(0, gl::TEXTURE_RECTANGLE_ARB);
}
...
bool CDDSImage::upload_textureCubemap()
{
    ...
    for (unsigned int n = 0; n < 6; n++)
    {
        ...
        target = gl::TEXTURE_CUBE_MAP_POSITIVE_X + n;
...
inline unsigned int CDDSImage::size_dxtc(unsigned int width, unsigned int height)
{
    return ((width+3)/4)*((height+3)/4)*
        (m_format == gl::COMPRESSED_RGBA_S3TC_DXT1_EXT ? 8 : 16);   
}
